...
PBRPipeline::PBRPipeline(Camera *camera,Primitives* primitives) : camera(camera), primitives(primitives) {
...
void PBRPipeline::Init() {
    ...
    pbrInstanceShader.setInt("prefilterMap", 1);
    pbrInstanceShader.setInt("brdfLUT", 2);
    pbrInstanceShader.setInt("albedoMap", 3);
    pbrInstanceShader.setInt("normalMap", 4);
    pbrInstanceShader.setInt("metallicMap", 5);
    ...
    equirectangularToCubemapShader.setMatrix4("projection", false, glm::value_ptr(captureProjection));
    ...
    glBindFramebuffer(GL_FRAMEBUFFER, captureFBO);
    for (unsigned int i = 0; i < 6; ++i) {
        irradianceShader.setMatrix4("view", false, glm::value_ptr(captureViews[i]));
    ...
    prefilterShader.setMatrix4("projection", false, glm::value_ptr(captureProjection));
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_CUBE_MAP, envCubemap);
    ...
    for (unsigned int mip = 0; mip < maxMipLevels; ++mip) {
        ...
        for (unsigned int i = 0; i < 6; ++i) {
            prefilterShader.setMatrix4("view", false, glm::value_ptr(captureViews[i]));
