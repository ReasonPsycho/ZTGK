...
MaterialPBR::loadMaterialTextures(aiMaterial *mat, aiTextureType type, string typeName, Model *model) {
...
std::shared_ptr<Texture>
MaterialPBR::forceLoadMaterialTexture(string path, aiTextureType type, string typeName, Model *model) {
    std::shared_ptr<Texture> texture;
    // check if texture was loaded before and if so, continue to next iteration: skip loading a new texture
    bool skip = false;
    for (unsigned int j = 0; j < model->textureCatalogue.size(); j++) {
...
MaterialPBR::MaterialPBR(aiMaterial *material, Model *model) {
...
// 4. roughness maps
 ...
 void MaterialPBR::loadMaterial(Shader* shader) {
...
MaterialPBR::MaterialPBR(GLubyte *color) {
