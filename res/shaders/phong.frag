...
struct TangentData{ // So it's easier to operate on
    vec3 ViewPos;
    vec3 FragPos;
    mat3 TBN;//pNo light position but TBN since fuck transporting 20 lights pos that are allready in SSBO
};
...
void main()
{
    if (isInFogOfWar){
        ...
        result = mix(result, vec3(colorMask) * color_mask_color(), colorMask.a);
...
vec3 CalcPointLight(PointLight light, vec3 normal, vec3 worldPos, vec3 viewDir, int lightIndex, vec3 diffuse, vec3 specular)
{
    ...
    if (distance < light.position.w){
        ...
        vec3  rim           = rim_intensity * vec3(light.diffuse) * rim_multiplier;
        ...
        return (df * diffuse_color + sf * specular_color) * shadow + rim;//Prob also need to style shadow or just use hard ones
    }
    return vec3(0);
}
