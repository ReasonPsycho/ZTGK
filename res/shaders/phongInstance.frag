...
void main()
{
    ...
    vec3 normal = texture(material.normalTextureArray, vec3(texCoords,0)).rgb;
    ...
    if (currentWallData[1] == 1){
        ...
        result = 0.2f * vec3(texture(material.diffuseTextureArray, vec3(texCoords, float(currentWallData[0]))));//We do be calculating ambient here
        int index = 0;
...
vec3 CalcDirLight(DirLight light, vec3 normal, vec3 viewDir, int lightIndex)
{
    ...
    // combine results
    vec3 diffuse = vec3(light.diffuse) * diff * vec3(texture(material.diffuseTextureArray, vec3(fs_in.TexCoords, float(currentWallData[0]))));
    vec3 specular = vec3(light.specular) * spec *vec3(texture(material.specularTextureArray, vec3(fs_in.TexCoords,0)));
...
vec3 CalcPointLight(PointLight light, vec3 normal, vec3 worldPos, vec3 viewDir, int lightIndex)
{
    vec3 lightDir = normalize( fs_in.tangentData.TBN * vec3(light.position)  - worldPos);
    // diffuse shading
    float diff = max(dot(normal, lightDir), 0.0);
    // specular shading
    vec3 reflectDir = reflect(-lightDir, normal);
...
vec2 ParallaxMapping(vec2 texCoords, vec3 viewDir)
{
    ...
    float currentDepthMapValue =     texture(material.depthTextureArray, vec3(currentTexCoords, 0)).r;
    ...
    while(currentLayerDepth < currentDepthMapValue)
    {
        ...
        currentDepthMapValue =     texture(material.depthTextureArray, vec3(currentTexCoords, 0)).r;
    ...
    float beforeDepth =     texture(material.depthTextureArray, vec3(prevTexCoords, 0)).r- currentLayerDepth + layerDepth;
