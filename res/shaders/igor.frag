...
{
    return floor(value * toon_color_levels) * toon_scale_factor;
}
...
void main()
{
    ...
    vec3 result = vec3(0,0,0); //We do be calculating ambient here
    int index = 0;
    ...
    vec3 aaa = vec3( toonify(result.x), toonify(result.y), toonify(result.z));
    aaa += 0.2f * vec3(texture(material.diffuse, vs_out.TexCoords));
    FragColor = vec4( aaa, 1.0);
}
